...
/**
 * Rasterizer for PNG input files (resizing).
 *
...
final class PngRasterizer implements RasterizerInterface
{
    ...
    {
        return 'png' === strtolower(pathinfo($inputFile, PATHINFO_EXTENSION));
    }
    ...
    public function rasterizeToPng(string $inputFile, int $size, string $targetPngFile): void
    {
        foreach ($this->adapters as $adapter) {
            if ($adapter->isAvailable()) {
                try {
                    $this->logger->debug(sprintf('Resizing PNG with %s', $adapter::class));
        ...
        throw new RasterizerUnavailableException('No suitable PNG rasterizer found (checked: ext-imagick, ext-gd).');
    }
}
